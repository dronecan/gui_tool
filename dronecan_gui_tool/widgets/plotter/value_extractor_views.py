...
def _make_expression_completer(owner, data_type):
    ...
    if isinstance(data_type, str):
        data_type = dronecan.TYPENAMES[data_type]
...
class NewValueExtractorWindow(QDialog):
    ...
    def _on_ok(self):
        ...
        try:
            data_type = dronecan.TYPENAMES[self._type_selector.currentText()]
...
class ExtractorWidget(QWidget):
    def __init__(self, parent, model):
        ...
        self._delete_button = make_icon_button('fa6s.trash', 'Remove this extractor', self, on_clicked=self._do_remove)
